...
호출규약은 엄청 많은데 인수관리가 어떻게 되는지 여기서 살펴보려함
...
+ [x86 에서만 가능하고](https://stackoverflow.com/questions/52713849/why-does-gcc-ignore-attribute-stdcall) WinApi 에서 주로 사용됨
  + 왜냐하면 x64 부터는 register 에 먼저 인자를 넣는 것을 선호하기 때문.

+ 인수는 오른쪽에서 왼쪽 순서대로 들어감

+ __인수정리를 Callee 가 함__
...
+ MSVN 에서 실험한 결과는
  + x86 에선 Callee 가 정리하고 ```...``` 를 붙여 명시적으로 쓰면 Caller 가 정리함
  +  x64 에선 Caller 레지스터를 쓰다가 모자라면 Caller 가 Prolog 에서 미리 만든 자리에 마치 Push 처럼 집어넣음.